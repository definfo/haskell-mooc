...
get :: UpDown -> Int
get (MkUpCounter n) = n
get (MkDownCounter n) = n

-- tick increases an increasing counter by one or decreases a
-- decreasing counter by one
tick :: UpDown -> UpDown
tick (MkUpCounter n) = MkUpCounter (n + 1)
tick (MkDownCounter n) = MkDownCounter (n - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (MkUpCounter n) = MkDownCounter n
toggle (MkDownCounter n) = MkUpCounter n
...
rgb (Invert col) = case rgb col of [r, g, b] -> [1 - r, 1 - g, 1 - b]
...
fromList xs = foldr (\(k, v) -> Pair k v) Empty xs
...
toBin n =
  if even n
  then O $ toBin $ div n 2
  else I $ toBin $ div (n - 1) 2
