...
data State = Start | AddedEgg | AddedFlour | AddedSugar | ToMix | ToBake | Error | Finished
  deriving (Eq,Show)

step :: State -> Event -> State
step Start AddEggs = AddedEgg
step Start _ = Error
step AddedEgg AddFlour = AddedFlour
step AddedEgg AddSugar = AddedSugar
step AddedEgg _ = Error
step AddedFlour AddSugar = ToMix
step AddedFlour _ = Error
step AddedSugar AddFlour = ToMix
step AddedSugar _ = Error
step ToMix Mix = ToBake
step ToMix _ = Error
step ToBake Bake = Finished
step ToBake _ = Error
step Error _ = Error
...
passwordAllowed pwd (ContainsSome xs) = any (`elem` pwd) xs
passwordAllowed pwd (DoesNotContain xs) = not $ any (`elem` pwd) xs
passwordAllowed pwd (And rq1 rq2) = passwordAllowed pwd rq1 && passwordAllowed pwd rq2
passwordAllowed pwd (Or rq1 rq2) = passwordAllowed pwd rq1 || passwordAllowed pwd rq2
...
data Arithmetic = BoxInt Integer
                | AddArith Arithmetic Arithmetic
                | MultiplyArith Arithmetic Arithmetic
  deriving Show
...
operation "+" arithl arithr = AddArith arithl arithr
operation "*" arithl arithr = MultiplyArith arithl arithr

evaluate :: Arithmetic -> Integer
evaluate (BoxInt n) = n
evaluate (AddArith arithl arithr) = evaluate arithl + evaluate arithr
evaluate (MultiplyArith arithl arithr) = evaluate arithl * evaluate arithr

render :: Arithmetic -> String
render (BoxInt n) = show n
render (AddArith arithl arithr) = "(" ++ render arithl ++ "+" ++ render arithr ++ ")"
render (MultiplyArith arithl arithr) = "(" ++ render arithl ++ "*" ++ render arithr ++ ")"
