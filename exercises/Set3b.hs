...
sums' i k sum = if k <= i then (sum + k) : sums' i (k + 1) (sum + k) else []
...
sumsOf' :: [Int] -> Int -> [Int]
sumsOf' [] _ = []
sumsOf' (s : xs) sum = (sum + s) : sumsOf' xs (sum + s)
...
mymaximum bigger initial (s : xs) =
    if bigger s initial then mymaximum bigger s xs
