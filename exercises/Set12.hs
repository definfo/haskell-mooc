...
-- β-reduction is not allowed here
...
instance Functor TwoList where
  ...
  fmap f (TwoNode a1 a2 tl) = TwoNode (f a1) (f a2) (fmap f tl)
...
count a x = length $ filter (== a) (toList x)
...
instance Foldable TwoList where
  ...
  foldr f x (TwoNode a1 a2 tl) = f a1 (f a2 (foldr f x tl))
...
sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node a lt rt) = sumTree lt <> a <> sumTree rt
