...
combine f (Picture p1) (Picture p2) = Picture (\px -> f (p1 px) (p2 px))
...
rectangle x0 y0 w h = Shape f
  where
    f (Coord x y)
      | interval x0 x (x0 + w) && interval y0 y (y0 + h) = True
      | otherwise = False

    interval a x b = x >= a && x < b
...
paintSolid color (Shape shape) (Picture pic) = Picture pic'
  where
    pic' coord
      | shape coord = color
      | otherwise = pic coord
...
paint (Picture pic) (Shape shape) (Picture base) = Picture pic'
  where
    pic' coord
      | shape coord = pic coord
...
examplePatterns = (paint (solid black) hat
...
instance Transform Fill where
  apply (Fill color) (Picture pic) = Picture (const color)

data Zoom = Zoom Int
  deriving (Show)

instance Transform Zoom where
  apply (Zoom z) (Picture pic) = Picture pic'
    where
      pic' (Coord x y) = pic $ Coord (div x z) (div y z)
...
instance Transform Flip where
  apply FlipX (Picture pic) = Picture pic'
    where
      pic' (Coord x y) = pic $ Coord (-x) y
  apply FlipY (Picture pic) = Picture pic'
    where
      pic' (Coord x y) = pic $ Coord x (-y)
  apply FlipXY (Picture pic) = Picture pic'
    where
      pic' (Coord x y) = pic $ Coord y x
...
instance Transform Blur where
  apply Blur (Picture pic) = Picture pic'
    where
      pic' = avg . map pic . coords

      coords (Coord x y) = [ Coord x y
                           , Coord (x - 1) y
                           , Coord (x + 1) y
                           , Coord x (y - 1)
                           , Coord x (y + 1)]
