...
workload nExercises hoursPerExercise
  ...
  | n < 10 = "Piece of cake!"
  | otherwise = "Ok."
  where n = nExercises * hoursPerExercise
...
countValid xs = foldr f 0 xs
  where f x | isValidBanknote x = (+ 1)
            | otherwise = id
        isValidBanknote x = ( length x >= 3 && (x !! 2) == (x !! 4) ) ||
                            ( length x >= 5 && (x !! 3) == (x !! 5) )
...
sumSuccess xs = foldr f (Left "no data") xs
  where f (Right x) (Left _) = Right x
        f (Right x) (Right n) = Right (n + x)
        f (Left _) e = e
...
open str (Closed code) | str == code = Open code
                       | otherwise = Closed code
open str (Open code) = Open code
...
changeCode str (Open code) = Open str
changeCode str (Closed code) = Closed code
...
instance Eq Text where
  (==) (Text a) (Text b) = trim a == trim b
    where trim = filter isNotSpace
          isNotSpace = \x -> not $ isSpace x
...
compose xs ys = foldr f [] xs
  where f (k, v) =
          case Map.lookup v (Map.fromList ys) of
            Nothing -> id
            Just v' -> ( (k, v') : )
...
permute p xs = permute' 0 p xs
  where permute' i indl s =
          case elemIndex i indl of
            Just i' -> (s !! i') : permute' (i + 1) indl s
            Nothing -> []