...
incrementKey key [] = []
incrementKey key (kv : kv_l) = if fst kv == key
                               then (key, snd kv + 1) : incrementKey key kv_l
                               else kv : incrementKey key kv_l
...
winner scores player1 player2 =
    if find_score player1 >= find_score player2 then player1 else player2
    where find_score k = Map.findWithDefault 0 k scores
...
transfer from to amount bank =
    if Map.notMember from bank ||
       ...
       amount < 0 ||
       fromMaybe 0 (Map.lookup from bank) < amount
    then bank
    else Map.adjustWithKey f_to to (Map.adjustWithKey f_from from bank)
    where f_from = const (+ (- amount))
          f_to = const (+ amount)
