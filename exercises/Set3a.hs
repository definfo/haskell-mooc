...
firstHalf s =
    if even l then take (div l 2) s
    else take (div l 2 + 1) s
    where l = length s
...
sumRights xs = sum (map either_sum xs)
    where either_sum = fromRight 0
...
multiCompose [] s = s
multiCompose xs s = multiCompose (init xs) (last xs s)
...
interpreter' (_ : commands) x y = "invalid command" : interpreter' commands x y
